...
namespace Rezet{
class Program{public static DiscordClient? Rezet;public static CommandsNextExtension? commands;
...
static async Task Main(string[] args){Console.ForegroundColor = ConsoleColor.White;Console.WriteLine("START ===================================================");Console.ResetColor();
var configPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "obj", "config.json");var configJson = await File.ReadAllTextAsync(configPath);var config = JsonSerializer.Deserialize<Config>(configJson);
            ...
            // ========== CLIENT:
var rzt = new DiscordConfiguration{Token = config?.Token,TokenType = TokenType.Bot,Intents = DiscordIntents.All,ReconnectIndefinitely = true,
GatewayCompressionLevel = GatewayCompressionLevel.Payload,MinimumLogLevel = Microsoft.Extensions.Logging.LogLevel.Error};
